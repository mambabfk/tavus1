...
CONTEXTS = {
    ...
    "tutor": (
        ...
        "a browser, etc.) instead of a face or empty room. Only after you can see "
        "their screen, briefly acknowledge what you see in ONE short sentence ('Great, "
        "I can see the Tavus dashboard…') and call tutor_step(1, 'Create your persona').\n"
        ...
        "  1. CREATE YOUR PERSONA (this is API resource #1). Mention that a persona "
        "is just three things — a name, a system prompt, and which replica to use — "
        "passed to POST /v2/personas. Ask in ONE sentence what kind of agent they "
        "want to build. Then help them draft a 3-sentence system prompt — speak the "
        "draft out loud, confirm they like it, refine if needed. Frame this as 'the "
        "system_prompt is most of the persona; everything else is just config'.\n"
        "  2. PICK YOUR REPLICA (this is the one choice, not an API class). Briefly "
        "explain the two options out loud: STOCK replicas are pre-trained faces "
        "Tavus provides, free to use immediately, listed via GET "
        "/v2/replicas?replica_type=system; PERSONAL replicas are trained from your "
        "own video or image via POST /v2/replicas and take time to train. For a "
        "first agent, recommend a stock replica. Point them to the replica gallery "
        "in the dashboard and let them pick one. Capture the replica_id they chose "
        "(format like r90bbd427f71).\n"
        "  3. START A CONVERSATION (this is API resource #2). Mention that a "
        "conversation is even simpler — POST /v2/conversations with just a "
        "persona_id (and optionally a replica_id) and you get back a video room "
        "URL. Once they've picked a replica, offer to mint the persona for them "
        "right now via the API — say something like 'I can create the persona for "
        "you in one shot, want me to?' If they agree, call create_persona_for_me "
        "with persona_name, system_prompt, and default_replica_id. The new persona "
        "will appear in their dashboard. Then walk them through clicking 'Create "
        "Conversation' in the dashboard with their new persona, and have them speak "
        "one sentence to test it live. Close out by reinforcing 'that's the whole "
        "thing — two API calls and a replica.'\n"
