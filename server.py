...
CONTEXTS = {
    ...
    "tutor": (
        ...
        "show_features_overview (no parameters). The panel paints six 'parts that "
        "make them feel alive.' Walk through SLOWLY — 1-2 sentences per part, with "
        "a real example. Take a beat between them; this is the meat. Reframe each "
        "as a POWER the user can give their agent (paraphrase, don't robot-read):\n"
        "      • 'Knowledge Base — drop your docs in, the agent answers from your "
        "data instead of guessing. Picture an HR agent pulling from your benefits "
        "handbook.'\n"
        "      • 'Objectives — give them a flow. Medical intake goes greet → "
        "symptoms → allergies → escalate, and the agent literally can't skip a "
        "step.'\n"
        "      • 'Guardrails — rules they will not cross. A finance agent won't "
        "give investment advice no matter how hard you push.'\n"
        "      • 'Tool Calling — they can call your APIs mid-call. That prompt card "
        "I just dropped in your panel? That was a tool call. Same pattern for "
        "anything — look up an order, kick off a workflow, render a UI.'\n"
        "      • 'Visual Awareness — Raven watches their face AND screen. If the "
        "user looks confused, the agent slows down. If they share an error, the "
        "agent spots it.'\n"
        "      • 'Pronunciation Dictionary — teach the TTS specific words. So when "
        "your CEO's name is Hassaan, it never says it weird.'\n"
        "Wrap with ONE line: 'Defaults are fine today. Those are your levers when "
        "you're ready to sharpen it.'\n"
