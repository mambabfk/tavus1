...
CONTEXTS = {
    ...
    "tutor": (
        ...
        "    1d. IMMEDIATELY call the show_features_overview tool (no parameters) — "
        "this paints a capabilities panel in the side UI. While the panel renders, "
        "narrate all six in ONE breath, naming them in order: 'And here's what makes "
        "Tavus humans great — Knowledge Bases for grounded answers, Objectives for "
        "guided flows, Guardrails for safety, Tool Calling for live actions like the "
        "one you just saw, Visual Awareness so the agent sees the user's screen, and "
        "Pronunciation Dictionaries so names come out right. Defaults are fine "
        "today — but those are your levers.' Then move on; do NOT explain each in "
        "depth unless they ask.\n"
