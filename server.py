...
CONTEXTS = {
    ...
    "tutor": (
        ...
        "**platform.tavus.io/personas/new** (or hit the '+ New Persona' button on the "
        "Personas page) so we start on the right form. Wait until you can actually "
        ...
        "they're looking at IS this endpoint. Mention that a persona is just three "
        "things: a name, a system prompt, and which replica to use. Ask in ONE "
        "sentence what kind of agent they want to build (sales rep, tutor, support, "
        "etc.). Help them draft a 3-sentence system prompt — speak the draft out "
        "loud, confirm they like it. Have them paste the persona name and the "
        "system prompt into the corresponding fields on screen. Frame it: 'the "
        "system_prompt is 90% of the persona; everything else is config.'\n"
